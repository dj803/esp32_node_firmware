...
### D. Open stability investigations (1) — #78 + #96 SHIPPED in v0.4.28
...
### G. Docs / process / long-tail closure (6) — mostly validation-pending, not coding
  ...
  #97   Auto-OTA-during-cascade-recovery gate                                (filed 2026-04-29 PM — observed during v0.4.28 AP-cycle #2 testing: Bravo/Echo/Foxtrot were on v0.4.26, recovering from a fresh WiFi outage, and as soon as they reconnected their periodic OTA poll fired, pulled v0.4.27 from gh-pages, flashed, and rebooted — all while still inside the cascade-recovery window. Worked OK this time but raises three concerns: (1) heap state is unknown post-cascade — silent #78-style corruption could brick an OTA write; (2) OTA install during instability adds reboots, amplifying the disruption window; (3) a regression in the new firmware is much harder to roll back from a fleet that's mid-cascade than from a steady-state fleet. PROPOSED FIX: at top of `otaCheckNow()` (and any other OTA-poll trigger sites), skip if `_lastNetworkDisconnectMs != 0 && (millis() - _lastNetworkDisconnectMs) < OTA_POST_DISCONNECT_QUIET_MS` — same pattern as v0.4.28 cascade-window guard but with a longer window (~5 min). Cost: OTA pickup delayed ≤5 min after WiFi events; in practice OTAs are mostly operator-triggered so periodic-poll being slightly later is negligible. Bundle into v0.4.29 alongside #87/#88/#89 ranging UX work)

  Total open: 29  (A6 + B12 + C0 + D1 + E0 + F1 + G9) — +6 from Phase 2 R1 2026-04-28 evening (#86 / #87 / #88 / #89 / #90 / #91); +1 from morning power-restoration cascade event 2026-04-29 (#92); +1 from serial-silence design question 2026-04-29 (#93); +1 from autonomous-mode firmware patch 2026-04-29 (#94 — patches staged, validation pending); +1 from PIO upload Unicode bug 2026-04-29 (#95); +1 from long-outage AP-mode loop 2026-04-29 (#96 — patched + shipped in v0.4.28); +1 from auto-OTA-during-cascade-recovery 2026-04-29 PM (#97 — bundled to v0.4.29 candidate); -1 #54 RESOLVED 2026-04-29; -1 #78 RESOLVED 2026-04-29 in v0.4.28 (cascade-window guard validated across 3 AP-cycle reproduction tests + mass-USB-flash recovery validated #96 self-clear path); -1 #96 RESOLVED 2026-04-29 in v0.4.28 (sub-A AP-portal-restart pushes ap_recovered + sub-B mqttScheduleRestart idempotent debounce — both validated by 6/6 self-recovery on flash)
...
## RESOLVED (full text in archive)
  ...
  #54   Stack-canary build (CONFIG_FREERTOS_CHECK_STACKOVERFLOW=2)         (resolved 2026-04-29 with positive evidence — Charlie's 35 h+ canary soak on v0.4.20.0 with CONFIG_FREERTOS_CHECK_STACKOVERFLOW=2 survived BOTH the 2026-04-28 evening cascade AND the 2026-04-29 morning cascade WITHOUT firing the stack canary, AND the 2026-04-29 09:21 bench-debug AP-cycle event, AND the chip-id RTS reset. Charlie reflashed to v0.4.27 production 2026-04-29 to free COM5 for #78 work; surveillance closed with strong heap-corruption confirmation, redirecting #78 fix away from stack-size territory)
